...
```{r}
# Load species data
GRBI <- readxl::read_excel("../data/RAPPORT QO_SOS-POP SCF_GRBI.xlsx", sheet = 2)
```
...
```{r}
# Environmental data
climatePresent <- readRDS("../data/bioclim_sQ.RDS")
climatePresent <- raster::stack(climatePresent)

# Elevation data
elevation <- readRDS("../data/elev_sQ.RDS")
```
...
```{r}
explana_dat <- raster::stack(climatePresent, elevation)
names(explana_dat[[which(names(explana_dat)==names(elevation))]]) <- "elevation"
```

Null predictor for purely spatial model

```{r}
#values(explana_dat) <- 0
#explana_dat <- explana_dat[[c(1,2)]]
```

Test with scaled predictors
```{r}
values(explana_dat) <- scale(values(explana_dat))
```
...
```{r, warning = FALSE}
# Spatial model
modelPP <- ppSpace(y ~ 0, sPoints = GRBI_points,
                   explanaMesh = explana,
                   ppWeight = weight,
                   control.compute = list(waic = TRUE))
modelPPmany <- ppSpace(y ~ 0, sPoints = GRBI_points,
                   explanaMesh = explana,
                   ppWeight = weight,
                   many = TRUE,
                   control.compute = list(waic = TRUE))
