...
if(max(cutoff_AUC, na.rm=TRUE) > max(cutoff_nQuadWanted, na.rm=TRUE)){
    ...
    SDM.AUC(bestModel_AUC, newdata=explana_dat, BITH=BITH, RL_cutoff = bestCutoff, template = template, plot_prediction = TRUE, points = TRUE, main = "best model")
}else{
    bestCutoff <- RangeLimit[which.max(cutoff_nQuadWanted)]
    SDM.AUC(bestModel_nQuadWanted, newdata=explana_dat, BITH=BITH, RL_cutoff = bestCutoff, template = template, plot_prediction = TRUE, points = TRUE, main = "best model")
}
```
