...
set.seed(123)
n.quad = c(1000, 10000, 100000, 1000000, 500000) 
cov <- c("temp * temp2 * prec + elevation + abie.balPropBiomass * abie.balBiomass")
...
plot(n.quad, nquadWanted, log = "x", type = "o", main = "overlap")
plot(n.quad, nquadWanted_noOverlap, log = "x", type = "o", main = "noOverlap")
```
...
Printing the summary of the best fit model according to the estimation of the loglikelihood within a standard error of two of its true value:
```{r summary best model nquadwanted, echo=FALSE, eval=TRUE}
if(max(nquadWanted, na.rm=TRUE) > max(nquadWanted_noOverlap, na.rm=TRUE)){
   cat("The best fitting model is with overlap of quadrature points with observations and with ", n.quad[which.max(nquadWanted)], " quad. points\n")
    bestModel_nQuadWanted <- get(paste0("model_", which.max(nquadWanted)))
    summary(bestModel_nQuadWanted)
}else{
    cat("The best fitting model is without overlap of quadrature points with observations and with ", n.quad[which.max(nquadWanted_noOverlap)], " quad. points\n")
    bestModel_nQuadWanted <- get(paste0("model_noOverlap_", which.max(nquadWanted_noOverlap)))
    summary(bestModel_nQuadWanted)
}
```
...
RangeLimit <- c(0.01, 0.03, 0.05, 0.07, 0.09, 0.11, 0.15, 0.2, 0.5)
cutoff_AUC <- c()
...
if(max(cutoff_AUC, na.rm=TRUE) > max(cutoff_nQuadWanted, na.rm=TRUE)){
    ...
    SDM.AUC(bestModel_AUC, newdata=explana_dat, BITH=BITH, RL_cutoff = bestCutoff, template = template, plot_prediction = TRUE, main = "best model")
}else{
    bestCutoff <- RangeLimit[which.max(cutoff_nQuadWanted)]
    SDM.AUC(bestModel_nQuadWanted, newdata=explana_dat, BITH=BITH, RL_cutoff = bestCutoff, template = template, plot_prediction = TRUE, main = "best model")
}
