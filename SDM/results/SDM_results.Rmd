...
# Colors
cols <- c(QC = "blue4", RL = "green4", ET = "brown3")
...
<div class = "col-md-6">
![](/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/BITH_RCP54_QC.gif)
</div>
...
```{r fig.2, fig.align='center', out.width = "100%", fig.asp = .4, echo=FALSE}
# Organize data frame
...
pn_QC <- ggplot(n_QC, aes(x=Time, y=n, group=scenario, linetype = scenario)) +
           geom_smooth(color=cols["QC"], se=FALSE, size=0.5, span = 0.7) +
...
pn_RL <- ggplot(n_RL, aes(x=Time, y=n, group=scenario, linetype = scenario)) +
           geom_smooth(color=cols["RL"], se=FALSE, size=0.5, span = 0.7) +
           theme_classic() +
           theme(legend.position = c(.95, .95), legend.justification = c("right", "top"), legend.key.size = unit(0.5, 'cm'), legend.text = element_text(size=7), legend.title=element_blank()) 

pn_ET <- ggplot(n_ET, aes(x=Time, y=n, group=scenario, linetype = scenario)) +
           geom_smooth(color=cols["ET"], se=FALSE, size=0.5, span = 0.7) +
           scale_y_continuous(name="Number of patches") +
           theme_classic() +
           theme(legend.position = c(.95, .95), legend.justification = c("right", "top"), legend.key.size = unit(0.5, 'cm'), legend.text = element_text(size=7), legend.title=element_blank()) 
...
## **Fig 3**. Change in spatial configuration of the distribution

```{r fig.3, fig.align='center', out.width = "100%", echo=FALSE}
#### Organize data frame ####
...
# # Area QC
parea_QC <- ggplot(area_QC, aes(fill=scenario, y=(area), x=Time)) +
                geom_violin(position="dodge", alpha=0.5, trim = TRUE, adjust = 0.5) +
                theme_classic() +
                ylab("Area (km2)") +
                ggtitle("QC")
# # Area ET
parea_ET <- ggplot(area_ET, aes(fill=scenario, y=(area), x=Time)) +
                geom_violin(position="dodge", alpha=0.5, trim = TRUE, adjust = 0.5) +
                theme_classic() +
                ylab("Area (km2)") +
                ggtitle("ET")
# # Area RL
parea_RL <- ggplot(area_RL, aes(fill=scenario, y=(area), x=Time)) +
                geom_violin(position="dodge", alpha=0.5, trim = TRUE, adjust = 0.5) +
                theme_classic() +
                ylab("Area (km2)") +
                ggtitle("RL")
...
# # Distance QC
pdist_QC <- ggplot(dist_QC, aes(fill=scenario, y=(dist), x=Time)) +
                geom_violin(position="dodge", alpha=0.5, trim = TRUE, adjust = 0.5) +
                theme_classic() +
                ylab("Distance (km)") +
                ggtitle("QC")
# # Distance ET
pdist_ET <- ggplot(dist_ET, aes(fill=scenario, y=(dist), x=Time)) +
                geom_violin(position="dodge", alpha=0.5, trim = TRUE, adjust = 0.5) +
                theme_classic() +
                ylab("Distance (km)") +
                ggtitle("ET")
# # Distance RL
pdist_RL <- ggplot(dist_RL, aes(fill=scenario, y=(dist), x=Time)) +
                geom_violin(position="dodge", alpha=0.5, trim = TRUE, adjust = 0.5) +
                theme_classic() +
                ylab("Distance (km)") +
                ggtitle("RL")
...
**Gig. 3**: Density distribution of area and inter-patch distance through time for climate forcing and habitat scenarios.


## **Fig 4**. Metapopulation effect
...
plot.capacity <- function(cap, totalArea, col = "black", main = ""){  
    ...
    p_cap <- ggplot(data=totalArea, aes(x=Time, y=totalArea/totalArea[1])) +
                geom_smooth(aes(weight=weight), se=FALSE, size=0.5, color=cols["RL"], span = 0.7) +
...
pcap_QC_RCP45 <- plot.capacity(capacity_QC[capacity_QC$scenario=="RCP4.5",], totalArea_QC[totalArea_QC$scenario=="RCP4.5",], col = cols["QC"], main = "QC")
pcap_ET_RCP45 <- plot.capacity(capacity_ET[capacity_ET$scenario=="RCP4.5",], totalArea_ET[totalArea_ET$scenario=="RCP4.5",], col = cols["ET"], main = "ET")
pcap_RL_RCP45 <- plot.capacity(capacity_RL[capacity_RL$scenario=="RCP4.5",], totalArea_RL[totalArea_RL$scenario=="RCP4.5",], col = cols["RL"], main = "RL")

pcap_QC_biomass <- plot.capacity(capacity_QC[capacity_QC$scenario=="biomass",], totalArea_QC[totalArea_QC$scenario=="biomass",], col = cols["QC"], main = "QC")
pcap_ET_biomass <- plot.capacity(capacity_ET[capacity_ET$scenario=="biomass",], totalArea_ET[totalArea_ET$scenario=="biomass",], col = cols["ET"], main = "ET")
pcap_RL_biomass <- plot.capacity(capacity_RL[capacity_RL$scenario=="biomass",], totalArea_RL[totalArea_RL$scenario=="biomass",], col = cols["RL"], main = "RL")

#### Arrange plots ####

ggarrange(pcap_QC_RCP45 + rremove("xlab"), pcap_ET_RCP45 + rremove("legend") + rremove("xlab"), pcap_RL_RCP45 + rremove("legend") + rremove("xlab"), pcap_QC_biomass + rremove("legend"), pcap_ET_biomass + rremove("legend"), pcap_RL_biomass + rremove("legend"), align = "h", ncol = 3, nrow = 2)
```
