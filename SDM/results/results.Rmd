...
### Predictions of Bicknell's thrush breeding range

```{r 4 range map, fig.align='center', out.width = "100%", out.height = "200%", echo=FALSE}
plot.map3()
```

**Fig 1**. Bicknell's thrush predicted breeding range. Breeding range is presented for A) current climatic conditions, for B) an increase of 2°c, and for C) an increase of 4°C.
...
**Fig. 2**. Bicknell's thrush breeding range distribution change in response to climate warming. 
...
# Compute capacities
if(!file.exists("/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_QC.RDS")) compute.capacity(metrics_QC, cc, totalArea_QC, file.name = "/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_QC.RDS")
if(!file.exists("/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_M.RDS")) compute.capacity(metrics_M, cc, totalArea_M, file.name = "/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_M.RDS")
if(!file.exists("/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_ET.RDS")) compute.capacity(metrics_ET, cc, totalArea_ET, file.name = "/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_ET.RDS")
```

```{r capacity vs. area, fig.align='center', out.width = "100%", fig.asp = .5, echo=FALSE, eval=TRUE}

# Function to return capacity plot
plot.capacity <- function(cc, totalArea, file.name, main = "", alpha = c(1, 5, 50, 200, 500)){
...
# 
p_capQC <- plot.capacity(cc, totalArea_QC, main = "Québec", file.name = "/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_QC.RDS")
p_capM <- plot.capacity(cc, totalArea_M, main = "Forêt Montmorency", file.name = "/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_M.RDS")
p_capET <- plot.capacity(cc, totalArea_ET, main = "Eastern Townships", file.name = "/Users/victorcameron/Documents/Git/Metapop_ms/SDM/results/capacities_ET.RDS")

ggarrange(p_capQC + rremove("xlab"), p_capM, p_capET + rremove("xlab"), ncol = 3, align = "h")
````

Metapopulation capacities are shown for alpha values of 1, 5, 50, 200, and 500km.
```{r metapop effect, fig.align='center', out.width = "100%", out.height = "75%", fig.asp = .6, echo=FALSE, eval=FALSE}
...
**Fig. 3**. Metapopulation effect. Habitat amount (solid line) and metapopulation capacity in response to climate warming are presented for regions of A) Québec, B) Forêt Montmorency, and C) Eastern Townships.

Change in habitat amount hides changes in landscape connectivity. Metapopulation capacity shows that the impact of landscape spatial configuration has effects on species persistence that is supperior to what of habitat amount alone. The metapop effect illustartes the non linearity between changes in habitat availability and species perrsistence in the landscape.

## Mismatch

Habitat (type of forest cover) proportion contained within the Bicknell's thrush projected breeding range for climate warming scenarios.

Point of this figure: to illustrate that the response lag of a lower trophic position creates a mismatch between its distribution and that of climatic conditions. Ultimately, Metapop theory predicts that the mismatch can alter the extent and the position of the top trophic level distribution limits, therefore affecting top trophic level response to climate warming.
...
par(mfrow = c(1,3))
barplot(t(t_GRBI_QC), beside = TRUE, legend = TRUE, main = "Québec", ylab = "Habitat proportion")
...
**Fig. 4**. Habitat - climate mismatch.

Examination of the mismatch between habitat and species abiotic niche (climate) shows an increase at the Québec scale of habitat amount (expansion) for an increase of 2°C. That is because previously unsuitable habitat is added to the breeding range. It is observable by the greater increase of the breeding range in the mixed forest than in the boreal forest. However, the effects of loss and fragmentation of previously suitable habitat is hidden by the breeding range expansion to new suitable patches.

Mismatch affects not only the proportion of habitat types contained within bicknell's thrush breeding range, but also the change in proportions of habitat types contained within the breeding range. At the scale of Québec, the change in proportions is of:

**Table. 1**. Proportion of habitat types contained within bicknell's thrush breeding range for climate warming scenarios.

```{r prop change QC, fig.align='center', echo=TRUE}
# Change in habitats proportions within the breeding range
...
Breeding range contraction is shown by the greater decrease in contained proportion of mixed and temperate forest types than boreal. That is the expression of the habitat-climate (niche) mismatch. In other words, the result of the forest habitat incapacity to respond to climate warming.
