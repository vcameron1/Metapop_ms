...
p_area <- ggplot(data=totalArea_dupl, aes(x=temp, y=totalArea, group=id, color=id)) +
          ...
          scale_y_continuous(name="Habitat size for Québec \n& F. Montmorency (km2)", sec.axis = sec_axis(~./coeff, name="Habitat size for \nEastern Townships (km2)")) +
          theme_classic() +
          theme(legend.position = c(.95, .95), legend.justification = c("right", "top"), legend.key.size = unit(0.5, 'cm'), legend.text = element_text(size=7), legend.title=element_blank())
...
Results at the scale of Québec as well as for the Forêt Montmorency present range expansion hiding the simulataneous range contraction. In the Eastern Townships, effects of range contraction are clearly observable.

The figure presents counter intuitive results: a warming of the climate leads to i) population loss (fewer occupied patches), ii) an increase in total area occupied and iii) almost perfectly constant density distribution of patch area. **Only the loss of patches of all size and the increase in size of few very large patches may explain the constant density distribution**.

These confounding results are the result of the distribution expansion to previously unsuitable habitat.

## Metapop effect

Habitat availability vs. landscape connectivity (capacity)

```{r capacity vs. area, fig.align='center', out.width = "100%", out.height = "75%", fig.asp = .6, echo=FALSE}

# Function to return capacity plots
plot.capacity <- function(metrics, cc, totalArea, alpha = c(1, 2, 4, 8, 16)){
...
# 
p_capQC <- plot.capacity(metrics_QC, cc, totalArea_QC)
p_capM <- plot.capacity(metrics_M, cc, totalArea_M)
p_capET <- plot.capacity(metrics_ET, cc)
...
ggarrange(pMeta_QC, pMeta_ET, pMeta_M, ncol = 3)
```

## Mismatch

Proportion of habitat (type of forest cover) occupied is presented for climate warming scenarios.
...
Examination of the mismatch between habitat and species abiotic niche shows an increase at the Québec scale of habitat occupancy (through range expansion). That is because newly suitable habitat is added to the distribution. However, the effects of loss of previously occupied habitat is hidden by the range expansion.
