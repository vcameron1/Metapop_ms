...
GRBI2 <- GRBI[GRBI$PRÉCISION == "S",]

# Remove recordings of absences
GRBI2 <- GRBI2[GRBI2$O_CODEATLA != "0",]

# Select presence in habitat, 
#GRBI2 <- GRBI2[GRBI2$O_CODEATLA == c("H"),]

# One observation per coordinate per year
annee <- unique(GRBI2$ANNEE)
annee <- annee[annee >= 2000]
GRBI2$lat <- round(GRBI2$LATITUDE, 2)
GRBI2$lon <- round(GRBI2$LONGITUDE, 2)

GRBI3 <- data.frame()
for(i in sort(annee)){
  for(j in unique(GRBI2$lat)){
    for(k in unique(GRBI2$lon)){
      obs <- which(GRBI2$ANNEE == i &
                     GRBI2$lat == j &
                     GRBI2$lon == k)
      
      if(length(obs)>0) GRBI3 <- rbind(GRBI3,GRBI2[obs[1],])
    }
  }
  cat(i, ": complété\n")
}
sunflowerplot(GRBI3$LONGITUDE, GRBI3$LATITUDE)
sunflowerplot(round(GRBI3$LONGITUDE,1), round(GRBI3$LATITUDE,1))
...
for(j in unique(GRBI2$lat)){
  for(k in unique(GRBI2$lon)){
    obs <- which(GRBI2$lat == j & GRBI2$lon == k)
      
    if(length(obs)>0) GRBI4 <- rbind(GRBI4,GRBI2[obs[1],])
  }
}
sunflowerplot(GRBI4$LONGITUDE, GRBI4$LATITUDE)
GRBI=GRBI4

# One observation per relevé
GRBI2 <- aggregate(cbind(LATITUDE, LONGITUDE, ANNEE) ~ `No RELEVÉ SOS-POP`, data = GRBI2, FUN = mean)
...
sort(table(GRBI3$LATITUDE))
sort(table(GRBI3$LONGITUDE))
GRBI=GRBI2
GRBI=GRBI3
```
...
# elevation <- raster::aggregate(elevation, fact = 4)
# explana_dat <- raster::stack(elevation)
...
rasterForPoly <- raster::aggregate(elevation, fact = 8)
...
explana <- explanaMesh(sPoly = spacePoly, 
                       mesh = Mesh, 
                       X = stack(stack(climatePresent), elevation, forestCover))
...
```{r, warning = FALSE}
# Spatial model
modelPPmany1 <- ppSpace(y ~ 0, sPoints = GRBI_points,
                   explanaMesh = explana,
                   ppWeight = weight,
                   many = TRUE,
                   control.compute = list(waic = TRUE))
modelPPmany2 <- ppSpace(y ~ 0, sPoints = GRBI_points,
                   explanaMesh = explana,
                   ppWeight = weight,
                   ...
                   many = TRUE,
                   control.compute = list(waic = TRUE))
modelPPmany3 <- ppSpace(y ~ 0, sPoints = GRBI_points,
                   explanaMesh = explana,
                   ppWeight = weight,
                   ...
                   many = TRUE,
                   control.compute = list(waic = TRUE))
modelPPmany4 <- ppSpace(y ~ 0, sPoints = GRBI_points,
                   explanaMesh = explana,
                   ppWeight = weight,
                   ...
                   many = TRUE,
                   control.compute = list(waic = TRUE))
modelPPmany5 <- ppSpace(y ~ 0, sPoints = GRBI_points,
                   explanaMesh = explana,
                   ppWeight = weight,
...
```{r}
waic <- sapply(mget(paste0("modelPPmany", 1:2)), function(x) x$waic$waic)
names(waic) <- paste0("model", 1:2)
waic

summary(modelPP) 
modelPPmany$waic$waic
```
...
curve(summary(modelPP)$coefficients["elevation",1]*x + summary(modelPP)$coefficients["Intercept",1], 0, max(values(explana_dat[["elevation"]]), na.rm = TRUE),
